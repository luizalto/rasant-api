...
def parse_subids_from_query(qs: str) -> Dict[str, Optional[str]]:
    ...
    for i in range(5):
        key_idx = f"subIds[{i}]"
        if key_idx in q and q[key_idx]: out[f"sub_id{i+1}"] = q[key_idx][0]
    if "subIds" in q:
        vals = q["subIds"]
...
def resolve_short_link(short_url: str, max_hops: int = 1) -> Tuple[str, Dict[str, Optional[str]]]:
    ...
    try:
        ...
        if 300 <= resp.status_code < 400 and "Location" in resp.headers:
            location = resp.headers["Location"]; current = urllib.parse.urljoin(current, location); final_url = current
    except Exception: final_url = short_url
    parsed = urllib.parse.urlsplit(final_url); subids = parse_subids_from_query(parsed.query)
...
def _load_te_if_available():
    ...
    if os.path.exists(TE_STATS_PATH):
        try:
            ...
            _TE_STATS = obj.get("stats", {}); _TE_PRIORS = obj.get("priors", {})
...
def _norm_text(s: str) -> str:
    s = (s or "").strip().lower()
    import unicodedata; s = unicodedata.normalize("NFKD", s)
    ...
    try:
        ...
        if os.path.exists(MODEL_PATH):
            ...
            if os.path.exists(MODEL_META_PATH):
                ...
                _model_classes = ["desinteressado","comprou"]; _comprou_threshold = 0.5
                _thresholds = {}; _FEATURE_NAMES_FROM_META = []; _CAT_IDX_FROM_META = []
...
def health():
    ...
    return {
        ...
        "comprou_threshold_prod": _comprou_threshold, "thr_atc": (_thresholds or {}).get("atc_high"),
        "thr_qvc": (_thresholds or {}).get("qvc_mid"), "n_features_meta": len(_FEATURE_NAMES_FROM_META),
...
def track_number_and_redirect(request: Request, full_path: str = Path(...), cat: Optional[str] = Query(None)):
    ...
    external_id_fallback  = _sha256_lower(f"{fbp_cookie}|{user_agent}|{ip_addr}")
...
async def admin_upload_model(
    ...
    return {"ok": True, "saved": {"model": MODEL_PATH, "meta": (MODEL_META_PATH if up_meta else None)},
            "model_loaded": bool(_model), "comprou_threshold_prod": _comprou_threshold}
...
def admin_test_event(
    ...
    now_ts = int(time.time()); evt_time = int(event_time) if event_time else now_ts
    ...
    payload = {"data": [{
        "event_name": event_name, "event_time": evt_time, "event_id": eid,
